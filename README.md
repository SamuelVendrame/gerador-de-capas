...
O projeto também não foi pensado pra escalar, o foco foi cumprir bem o que o PRD pedia rodando localmente, não construir algo pronto pra múltiplos usuários ou grande volume de dados. Isso aparece, por exemplo, na escolha de guardar o histórico em um arquivo JSON simples em vez de um banco de dados. Resolve bem o escopo de um app local de só um usuário, mas não seria a estrutura certa se o projeto precisasse crescer - mas não acredito que seria um problema em adaptar posteriormente.
