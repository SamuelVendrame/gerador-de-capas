...
import {
  LABELS_STATUS,
  formatarDataHora,
} from "~/composables/useFormatacao";
...
<template>
  <DetalheCapa
    ...
    @fechar="registroAberto = null"
  />

  <div v-else class="painel-historico">
    <p v-if="carregando" class="estado-mensagem">
      Carregando...
    </p>
    ...
    >
      ...
      {{ resumo.emProcessamento }} em processamento ·
      {{ resumo.comFalha }} com falha — clique para abrir
    </p>
    ...
    >
      <img
        v-if="registro.caminhoImagem"
        :src="registro.caminhoImagem"
        class="thumb"
        alt="Capa gerada"
      />

      <div v-else class="thumb thumb-vazia"></div>

      <div class="conteudo-item">
        <div class="info">
          <strong class="titulo-registro">
            {{ registro.titulo }}
          </strong>

          <div class="infos-livro">
            <span class="info-livro">
              {{ registro.autor }} -
            </span>
        ...
        <div class="linha-status">
          <div
            class="status"
            :class="registro.status"
          >
            <span class="icone-status">
              <svg
                v-if="registro.status === 'concluido'"
                viewBox="0 0 24 24"
                width="12"
                height="12"
                fill="none"
              ...
              <svg
                v-else-if="registro.status === 'cancelado'"
                viewBox="0 0 24 24"
                width="12"
                height="12"
                fill="none"
        ...
        <div
          v-if="registro.status === 'cancelado'"
          class="bloco-erro"
          @click.stop
        >
          <p
            v-if="registro.motivoCancelamento"
            class="motivo"
          >
          ...
          <button
            ...
            @click="tentarNovamente(registro)"
          >
            <svg
              viewBox="0 0 24 24"
              width="14"
            ...
            >
              <path
                d="M21 12a9 9 0 0 1-9 9 9 9 0 0 1-9-9 9 9 0 0 1 9-9c2.52 0 4.93 1 6.74 2.74L21 8"
              />
              <path d="M21 3v5h-5" />
            </svg>
